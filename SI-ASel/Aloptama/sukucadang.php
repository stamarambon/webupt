...
function encrypt_pin($pin) {
    // Use a combination of hashing and encryption for better security
    $salt = 'suku_cadang_2024_secure_salt';
    $pepper = 'meteorologi_maritim_ambon_pepper';
    return hash('sha256', $salt . $pin . $pepper);
}

function verify_pin($input_pin, $stored_hash) {
    return hash_equals($stored_hash, encrypt_pin($input_pin));
}
...
function authenticate_user($pin) {
    $correct_pin = '97730';
    $encrypted_pin = encrypt_pin($correct_pin);
    
    if (verify_pin($pin, $encrypted_pin)) {
...
if (isset($_POST['pin_submit'])) {
    $input_pin = sanitize_input($_POST['pin']);
    if (authenticate_user($input_pin)) {
        // Redirect to prevent resubmission
        header('Location: ' . $_SERVER['PHP_SELF']);
        exit();
    } else {
        $pin_error = 'PIN tidak valid!';
    }
}

// Check authentication before showing content
$show_content = is_authenticated();
...
<body class="bg-light">
    ...
    <?php if (!$show_content): ?>
    <div class="modal fade show pin-modal" id="pinModal" tabindex="-1" style="display: block;" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">
                        <i class="fas fa-lock"></i> Autentikasi Keamanan
                    </h5>
                </div>
                <div class="modal-body text-center">
                    ...
                    <h6 class="mb-3">Masukkan PIN untuk mengakses sistem</h6>
                    <form method="POST" action="">
                        ...
                        <?php if (isset($pin_error)): ?>
                            <div class="alert alert-danger alert-sm">
                                <i class="fas fa-exclamation-triangle"></i> <?php echo htmlspecialchars($pin_error); ?>
                            </div>
                        <?php endif; ?>
                        <div class="d-grid">
                            <button type="submit" name="pin_submit" class="btn btn-primary">
                ...
                <div class="modal-footer text-center">
                    <small class="text-muted">
                        ...
                        Sistem Managemen Suku Cadang Meteorologi Maritim Ambon
                    </small>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (pinInput) {
                // Focus on PIN input when modal loads
                pinInput.focus();
                ...
                // Prevent form submission if PIN is not 5 digits
                const pinForm = pinInput.closest('form');
                if (pinForm) {
